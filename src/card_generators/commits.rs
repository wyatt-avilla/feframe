...
#[styled_component]
pub fn CommitRow(props: &RowProps) -> Html {
...
pub fn generate_commit_card() -> Html {
    let icon_svg = html! {
    ...
    let rows: Vec<String> = (1..15).map(|i| format!("commit {i}")).collect();
    let scroller_props = ScrollerProps {
        rows: rows
            .iter()
            .map(|row| html! { <CommitRow value={ row.clone() } /> })
            .collect(),
    };

    let title = String::from("Title generated in commits.rs");
